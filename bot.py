    ...
    if ticket_eintrag and ticket_eintrag.get("ki_aktiv", True) and message.content.strip() and bot.user not in message.mentions:
        ...
        async with message.channel.typing():
            ...
            if wissen:
                system += f"\n\nWissensdatenbank (nutze dies für Fakten, falls relevant):\n{wissen}"
            antwort = await frage_gemini(system, message.content, brain["conversation_history"])
            if not antwort.startswith("❌") and not antwort.startswith("🕐"):
                ...
                await message.channel.send(embed=ki_embed)
                await lerne(message.content, antwort, uid=str(message.author.id), channel=message.channel)
                if muss_eskalieren:
                    await team_pingen(message.channel, "Die KI ist sich unsicher und bittet um menschliche Hilfe.")
...
async def erstelle_ticket_kanal(guild: discord.Guild, ersteller: discord.Member, kategorie_label: str, beschreibung: str = "") -> discord.TextChannel | None:
    ...
    if beschreibung and daten.get("ticket_ki_aktiv", True) and GROQ_KEY:
        ...
        if wissen:
            system += f"\n\nWissensdatenbank (nutze dies für Fakten, falls relevant):\n{wissen}"
        erste_antwort = await frage_gemini(system, beschreibung)
        if not erste_antwort.startswith("❌"):
            muss_eskalieren = "ESKALIEREN" in erste_antwort
            antwort_ohne_marker = erste_antwort.replace("ESKALIEREN", "").strip()
            ki_embed = discord.Embed(description=antwort_ohne_marker[:2000], color=0xbf5fff)
            ki_embed.set_author(name="🧠 NEXUS Support-KI")
...
async def voice_text_zu_sprache(text: str, stimme: str) -> str:
    """Wandelt Text in eine MP3-Datei um (temporäre Datei) und gibt den Pfad zurück.
    Gibt None zurück, falls edge_tts nicht verfügbar ist oder die Synthese fehlschlägt."""
...
async def voice_sag(interaction: discord.Interaction, text: str):
    ...
    mp3_pfad = await voice_text_zu_sprache(text, stimme)
    if not mp3_pfad:
        await interaction.followup.send("❌ Sprachsynthese fehlgeschlagen.")
        return

    try:
        quelle = discord.FFmpegPCMAudio(mp3_pfad)
        vc.play(quelle, after=lambda e: os.remove(mp3_pfad) if os.path.exists(mp3_pfad) else None)
        await interaction.followup.send(f"🔊 Sage: *{text[:200]}*")
    except Exception as e:
        await interaction.followup.send(f"❌ Wiedergabe fehlgeschlagen (FFmpeg auf dem Server installiert?): {e}")
...
async def hilfe(interaction: discord.Interaction):
    embed = discord.Embed(title="📖 NEXUS COMMUNITY BOT v16.0 — Befehle", color=0x00f5ff)
    ...
    embed.add_field(name="🎫 KI-Tickets", value="`/ticket [beschreibung]` oder Dropdown-Panel nutzen\n`/team-rufen` — Support-Team aktiv holen\n`/ticket-ki-umschalten` `/close` (KI fasst zusammen)", inline=False)
    ...
    if ist_owner(interaction.user.id):
        embed.add_field(
            ...
            value=(
                ...
                "`/setup-ticket-team` `/setup-ticket-log`\n"
                "`/reaction-role-erstellen` `/reaction-role-hinzufügen`\n"
                "`/setup-ticket-log` `/ticket-ki-global-umschalten`"
            ),
            inline=False
        )
    await interaction.response.send_message(embed=embed, ephemeral=True)
