    ...
    if ticket_eintrag and ticket_eintrag.get("ki_aktiv", True) and message.content.strip() and bot.user not in message.mentions:
        ...
        async with message.channel.typing():
            ...
            system = (
                ...
                "Falls du das Problem nicht lösen kannst, sag das ehrlich und dass sich ein Mod melden wird."
            )
            if wissen:
                system += f"\n\nWissensdatenbank (nutze dies für Fakten, falls relevant):\n{wissen}"
            antwort = await frage_gemini(system, message.content, brain["conversation_history"])
            if not antwort.startswith("❌") and not antwort.startswith("🕐"):
                ki_embed = discord.Embed(description=antwort[:2000], color=0xbf5fff)
...
async def ticket(interaction: discord.Interaction, beschreibung: str = ""):
    ...
    badge_farben = {"Bug": 0xff3333, "Frage": 0x00f5ff, "Beschwerde": 0xff9900, "Vorschlag": 0x00ff88, "Allgemein": 0xbf5fff}
    embed = discord.Embed(
        title=f"🎫 Ticket — {kategorie}",
        description=f"{interaction.user.mention} — Beschreibe dein Anliegen, die KI antwortet automatisch wenn möglich.\n`/close` zum Schließen.",
        color=badge_farben.get(kategorie, 0x00f5ff)
    )
    if beschreibung:
        embed.add_field(name="Erste Beschreibung", value=beschreibung[:500], inline=False)
    await kanal.send(embed=embed)

    # KI gibt direkt eine erste Antwort, falls eine Beschreibung vorhanden ist
    if beschreibung and daten.get("ticket_ki_aktiv", True) and GROQ_KEY:
        ...
        if not erste_antwort.startswith("❌"):
            ki_embed = discord.Embed(description=erste_antwort[:2000], color=0xbf5fff)
            ki_embed.set_author(name="🧠 NEXUS Support-KI")
...
async def hilfe(interaction: discord.Interaction):
    embed = discord.Embed(title="📖 NEXUS COMMUNITY BOT v14.0 — Befehle", color=0x00f5ff)
    ...
    embed.add_field(name="🎫 KI-Tickets", value="`/ticket [beschreibung]` — KI kategorisiert + antwortet automatisch\n`/ticket-ki-umschalten` `/close` (KI fasst zusammen)", inline=False)
