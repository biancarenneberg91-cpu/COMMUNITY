...
def lade_daten():
    default = {
        ...
        # ── v21: ModMail — DM ans Team via Bot-Thread ──
        "modmail_kategorie_id": None,       # Discord-Kategorie-ID, in der ModMail-Kanäle entstehen
...
async def modmail_thread_oeffnen(nutzer: discord.User, erste_nachricht: str) -> discord.TextChannel | None:
    """Öffnet einen ModMail-Kanal im Server für eine DM. Gibt None zurück, falls
    bereits ein Thread offen ist oder die Konfiguration (Kategorie) fehlt."""
    daten = lade_daten()
    if str(nutzer.id) in daten.get("modmail_aktive_threads", {}):
        return None

    kategorie_id = daten.get("modmail_kategorie_id")
    if not kategorie_id:
        return None

    guild = None
    for g in bot.guilds:
        if g.get_channel(int(kategorie_id)):
            guild = g
            break
    if not guild:
        return None
    ...
    team_rolle_name = daten.get("modmail_team_rolle")
    if team_rolle_name:
    ...
    kanal_name = f"modmail-{nutzer.name.lower()}"[:90]
    try:
        kanal = await guild.create_text_channel(kanal_name, category=kategorie, overwrites=overwrites)
    except Exception as e:
        print(f"[ModMail] Konnte Kanal nicht erstellen: {e}")
        return None

    daten.setdefault("modmail_aktive_threads", {})
    daten["modmail_aktive_threads"][str(nutzer.id)] = str(kanal.id)
    speichere_daten(daten)

    embed = discord.Embed(
        title="📬 Neue ModMail-Anfrage",
        description=f"Von: {nutzer.mention} ({nutzer})\n\n{erste_nachricht[:1500]}",
        color=0x00f5ff,
        timestamp=datetime.datetime.utcnow()
    )
    embed.set_thumbnail(url=nutzer.display_avatar.url)
    embed.set_footer(text="Antworten hier im Kanal werden automatisch als DM weitergeleitet. /modmail-schließen zum Beenden.")
    await kanal.send(embed=embed)
    return kanal

async def modmail_an_nutzer_weiterleiten(kanal_id: str, autor: discord.Member, inhalt: str):
    """Leitet eine Nachricht aus dem Server-ModMail-Kanal als DM an den ursprünglichen Nutzer weiter."""
    daten = lade_daten()
    nutzer_id = next((uid for uid, kid in daten.get("modmail_aktive_threads", {}).items() if kid == kanal_id), None)
    if not nutzer_id:
        return False
    try:
        nutzer = await bot.fetch_user(int(nutzer_id))
        embed = discord.Embed(description=inhalt[:2000], color=0x00ff88)
        embed.set_author(name=f"Team-Antwort von {autor.display_name}", icon_url=autor.display_avatar.url)
...
async def on_message(message):
    ...
    if message.guild is None:
        ...
        # ModMail: Falls bereits ein Thread offen ist, geht JEDE DM dorthin —
        # die KI antwortet in diesem Fall NICHT, das Team übernimmt komplett.
        aktive_threads = daten.get("modmail_aktive_threads", {})
        uid_str = str(message.author.id)
        if uid_str in aktive_threads and message.content.strip():
            kanal_id = aktive_threads[uid_str]
            for guild in bot.guilds:
                kanal = guild.get_channel(int(kanal_id))
                if kanal:
                    embed = discord.Embed(description=message.content[:2000], color=0x00f5ff, timestamp=datetime.datetime.utcnow())
                    embed.set_author(name=str(message.author), icon_url=message.author.display_avatar.url)
                    try:
                        await kanal.send(embed=embed)
                    except Exception as e:
                        print(f"[ModMail] Konnte DM nicht weiterleiten: {e}")
                    break
            await bot.process_commands(message)
            return

        # ModMail-Trigger-Wort: öffnet einen neuen Thread statt der KI zu antworten
        trigger_wort = daten.get("modmail_trigger_wort", "modmail").lower()
        if message.content.strip().lower().startswith(trigger_wort):
            nachricht_ohne_trigger = message.content[len(trigger_wort):].strip(" :,-")
            kanal = await modmail_thread_oeffnen(message.author, nachricht_ohne_trigger or "(keine weitere Nachricht)")
            if kanal:
                await message.channel.send("📬 Deine Nachricht wurde an unser Team weitergeleitet! Du bekommst hier eine DM, sobald jemand antwortet.")
            else:
                await message.channel.send("⚠️ Konnte keinen ModMail-Thread öffnen — entweder hast du schon einen offenen, oder das Team hat ModMail noch nicht eingerichtet.")
            await bot.process_commands(message)
            return
...
Datum: {datetime.datetime.utcnow().strftime('%d.%m.%Y')}"""
    ...
    # ModMail-Rückrichtung: Nachrichten im ModMail-Kanal gehen als DM an den Nutzer.
    # Muss VOR Anti-Spam/XP laufen, damit Team-Antworten nicht versehentlich gefiltert werden.
    if message.channel.name.startswith("modmail-") and str(message.channel.id) in daten.get("modmail_aktive_threads", {}).values() and message.content.strip():
        await modmail_an_nutzer_weiterleiten(str(message.channel.id), message.author, message.content)
        await bot.process_commands(message)
        return
...
# ═══════════════════════════════════════════════════════════════
#  MODMAIL — Slash-Commands
# ═══════════════════════════════════════════════════════════════
@bot.tree.command(name="setup-modmail", description="[OWNER] ModMail einrichten: Kategorie + Team-Rolle festlegen")
@owner_only()
async def setup_modmail(interaction: discord.Interaction, kategorie: discord.CategoryChannel, team_rolle: discord.Role):
    daten = lade_daten()
    daten["modmail_kategorie_id"] = str(kategorie.id)
    daten["modmail_team_rolle"] = team_rolle.name
    speichere_daten(daten)
    trigger = daten.get("modmail_trigger_wort", "modmail")
    await interaction.response.send_message(
        f"✅ ModMail eingerichtet!\n"
        f"**Kategorie:** {kategorie.name}\n"
        f"**Team-Rolle:** {team_rolle.mention}\n\n"
        f"Nutzer schreiben dem Bot einfach eine DM beginnend mit **\"{trigger}\"** "
        f"(z.B. \"{trigger}: ich brauche Hilfe bei...\"), um einen Thread zu öffnen.\n"
        f"Mit `/modmail-trigger-wort` kannst du das Schlüsselwort ändern.",
        ephemeral=True
    )

@bot.tree.command(name="modmail-trigger-wort", description="[OWNER] Das Schlüsselwort ändern, das einen ModMail-Thread per DM öffnet")
@owner_only()
async def modmail_trigger_wort_cmd(interaction: discord.Interaction, wort: str):
    daten = lade_daten()
    daten["modmail_trigger_wort"] = wort.strip().lower()
    speichere_daten(daten)
    await interaction.response.send_message(f"✅ ModMail-Trigger-Wort ist jetzt: **{wort.strip().lower()}**", ephemeral=True)

@bot.tree.command(name="modmail-schließen", description="ModMail-Thread schließen (nur im ModMail-Kanal nutzbar)")
async def modmail_schliessen(interaction: discord.Interaction):
    daten = lade_daten()
    aktive_threads = daten.get("modmail_aktive_threads", {})
    nutzer_id = next((uid for uid, kid in aktive_threads.items() if kid == str(interaction.channel_id)), None)

    if not nutzer_id:
        await interaction.response.send_message("❌ Das ist kein aktiver ModMail-Kanal.", ephemeral=True)
        return

    await interaction.response.send_message("🔒 Schließe ModMail-Thread in 5 Sekunden...")

    try:
        nutzer = await bot.fetch_user(int(nutzer_id))
        abschluss_embed = discord.Embed(
            description="Dieses ModMail-Gespräch wurde vom Team geschlossen. Schreib uns gerne erneut, falls du weitere Fragen hast!",
            color=0x00ff88
        )
        await nutzer.send(embed=abschluss_embed)
    except Exception as e:
        print(f"[ModMail] Konnte Abschluss-DM nicht senden: {e}")

    del daten["modmail_aktive_threads"][nutzer_id]
    speichere_daten(daten)
    ...
    try:
        ...
        print(f"[ModMail] Konnte Kanal nicht löschen: {e}")

@bot.tree.command(name="modmail-liste", description="[OWNER] Alle aktiven ModMail-Threads anzeigen")
@owner_only()
async def modmail_liste(interaction: discord.Interaction):
    daten = lade_daten()
    aktive_threads = daten.get("modmail_aktive_threads", {})
    if not aktive_threads:
        await interaction.response.send_message("Keine aktiven ModMail-Threads.", ephemeral=True)
        return
    embed = discord.Embed(title="📬 Aktive ModMail-Threads", color=0x00f5ff)
